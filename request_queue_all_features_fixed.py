...
def _t(lang: str, key: str, **kw) -> str:
    ...
    T = {
        ...
        "check_dm_hint": {
            "hx": "DM check karo ✅ Bot ka reply private me aaya hoga. Agar PM open nahi hai toh bot ko private me open karke /start kar lo.",
            "hi": "DM check karo ✅ Bot ka reply private me aaya hoga. Agar PM open nahi hai toh bot ko private me open karke /start kar lo.",
        },
...
"
    ...
    if not req.get("silent"):
        await _dm_safe(
            context,
            user_id,
            ((_t(user_lang, "status_update", group=group_title, id=seq, item=req.get("item"), status=req.get("status")) + (f"\nReason: {_reason_label(req.get('reason'))}" if req.get('status')=='rejected' and req.get('reason') else ""))
             + (f"
Reason: {_reason_label(req.get('reason'))}" if req.get("status")=="rejected" and req.get("reason") else ""))
        )
