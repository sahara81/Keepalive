...
async def cmd_promostatus(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await reply_autodelete(update.message, context, f"Promo tag filter: {'ON' if state else 'OFF'}")
# ---------- REQUEST QUEUE + UPVOTE SYSTEM ----------

def _rq_norm(s: str) -> str:
    return " ".join((s or "").lower().strip().split())

def _rq_queue(context: ContextTypes.DEFAULT_TYPE):
    # chat-wide queue
    return context.chat_data.setdefault("request_queue", [])

def _rq_find_pending(queue, norm_item: str):
    for i, r in enumerate(queue):
        if r.get("norm") == norm_item and r.get("status") == "pending":
            return i, r
    return None, None

async def cmd_request(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/request <item> — Add a request. If same pending item exists, this acts as an upvote."""
    if not update.message:
        return
    chat = update.effective_chat
    user = update.effective_user
    ...
    norm = _rq_norm(item)
    queue = _rq_queue(context)

    # Duplicate pending item => upvote
    dup_i, dup_r = _rq_find_pending(queue, norm)
    if dup_r:
        voters = dup_r.get("voters") or []
        if user.id in voters:
            return await reply_autodelete(
                update.message,
                context,
                f"Already requested & already upvoted ✅\\nPosition: *#{dup_i + 1}*\\nVotes: *{dup_r.get('votes', 0)}*",
            )

        voters.append(user.id)
        dup_r["voters"] = voters
        dup_r["votes"] = int(dup_r.get("votes", 0)) + 1
        queue[dup_i] = dup_r
        context.chat_data["request_queue"] = queue
    ...
    add_log(context, f"REQUEST_ADD user={user.id} item={item!r} pos={len(queue)}")

    kb = InlineKeyboardMarkup([
        [
            InlineKeyboardButton("👍 Upvote", callback_data=f"request_upvote_{req_id}"),
    ...
    text = (
        ...
        "Votes: *0*\\n\\n"
        "_Admins: approve/reject. Members: upvote._"
    )
    await update.message.reply_text(text, reply_markup=kb, parse_mode="Markdown", disable_web_page_preview=True)

async def cmd_myrequests(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/myrequests — Show your requests in this chat."""
    if not update.message:
        return
    user = update.effective_user
    if not user:
    ...
    queue = _rq_queue(context)
    mine = [r for r in queue if r.get("by_id") == user.id]
    if not mine:
        return await reply_autodelete(update.message, context, "You have no requests in this chat.")

    lines = []
    for r in mine[-15:]:
        lines.append(f"- *{r.get('item')}* — `{r.get('status')}` — 👍 *{r.get('votes', 0)}*")
    await reply_autodelete(update.message, context, "Your Requests:\\n" + "\\n".join(lines))

async def cmd_queue(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/queue — Admins: show pending queue sorted by votes."""
    if not update.message:
        return
    if not (update.effective_user.id == OWNER_ID or await is_admin(update, context)):
        return await reply_autodelete(update.message, context, "Only admins allowed.")
    ...
    queue = _rq_queue(context)
    if not queue:
        return await reply_autodelete(update.message, context, "Queue is empty.")

    pending = [r for r in queue if r.get("status") == "pending"]
    done = [r for r in queue if r.get("status") != "pending"]

    pending_sorted = sorted(pending, key=lambda r: (-int(r.get("votes", 0)), int(r.get("created_at", 0))))

    lines = []
    if pending_sorted:
        lines.append("🟡 *Pending (sorted by votes)*")
        for i, r in enumerate(pending_sorted[:30], start=1):
            lines.append(f"{i}. *{r.get('item')}* — _{r.get('by_name')}_ — 👍 *{r.get('votes', 0)}*")
    if done:
        lines.append("")
        lines.append("🟢 *Completed/Rejected* (last 10)")
        for r in done[-10:]:
            lines.append(f"- *{r.get('item')}* — `{r.get('status')}`")

    await reply_autodelete(update.message, context, "\\n".join(lines))

async def cb_request(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle request buttons: upvote for all, approve/reject for admins."""
    query = update.callback_query
    if not query:
        return

    data = query.data or ""
    parts = data.split("_", 2)  # request, action, id
    if len(parts) < 3:
        await query.answer()
        return
    ...
    if req is None:
        await query.answer("Request not found.", show_alert=False)
        return

    # Upvote (anyone)
    if action == "upvote":
        user = query.from_user
        voters = req.get("voters") or []
        if user and user.id in voters:
            await query.answer("Already upvoted ✅", show_alert=False)
            return

        if user:
            voters.append(user.id)
        req["voters"] = voters
        req["votes"] = int(req.get("votes", 0)) + 1
        queue[idx] = req
        context.chat_data["request_queue"] = queue

        await query.answer("Upvoted 👍", show_alert=False)
        return

    # Approve/Reject (admins/owner only)
    if not (query.from_user and (query.from_user.id == OWNER_ID or await is_admin(update, context))):
        await query.answer("Admins only.", show_alert=False)
        return
    ...
    req["handled_by"] = query.from_user.id
    req["handled_at"] = int(time.time())
    add_log(context, f"REQUEST_{req['status'].upper()} by={query.from_user.id} req={req_id}")

    try:
        await context.bot.send_message(
            chat_id=query.message.chat_id,
            text=f"✅ Request *{req['item']}* — `{req['status']}`\\nBy: *{req['by_name']}*",
    ...
    context.chat_data["request_queue"] = [r for r in queue if r.get("id") != req_id]
...
def main():
    ...
    application.add_handler(CallbackQueryHandler(cb_request, pattern=r"^request_(?:upvote|approve|reject)_"))
