...
✅ Pehle yahi PM me /start karo
➡️ Phir group me /request bhejo

📌 Kaise use karna hai:
➡️ /request <item>  (GROUP)
   Example: /request Avengers React WKS
   • Group message auto-delete ho jayega
   • Status/confirmation tumhe DM me aayega

➡️ /myrequests  (DM)
   • Tumhari requests + status: pending / fulfilled / rejected
...
async def cmd_request(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not _cooldown_ok(context, user.id):
        await _dm_safe(context, user.id, "5 min ruk jao, phir request bhejna.")
        return
