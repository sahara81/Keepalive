...
async def cmd_promostatus(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async def cmd_request(update: Update, context: ContextTypes.DEFAULT_TYPE):
        """
        Add a new item to the request queue. Users can request items such as movies, songs or files.
        The bot prevents duplicates and informs users of their position in the queue. Each request
        generates an inline keyboard for admins with Approve/Reject buttons.
        """
        if not update.message:
            return
        # Ensure a description is provided
        if not context.args:
            return await reply_autodelete(update.message, context, "Usage: /request <item name>")
        description = " ".join(context.args).strip()
        # Retrieve the existing queue for this chat, or initialise it
        queue = context.chat_data.get("requests_queue", [])
        # Normalized key for duplicate detection / upvotes
        norm_desc = " ".join(description.lower().split())

        # If same request already exists (pending), treat as UPVOTE
        for idx, req in enumerate(queue):
            if req.get("norm") == norm_desc and req.get("status") == "pending":
                voters = req.get("voters", [])
                if requester.id in voters:
                    return await reply_autodelete(
                        update.message,
                        context,
                        f"Already voted 👍\n\nRequest: *{req.get('description')}*\nVotes: *{req.get('votes', 1)}*",
                    )

                voters.append(requester.id)
                req["voters"] = voters
                req["votes"] = int(req.get("votes", 1)) + 1
                context.chat_data["requests_queue"] = queue

                # Best-effort: update the original request post (if stored)
                try:
                    chat_id = req.get("announce_chat_id")
                    msg_id = req.get("announce_message_id")
                    if chat_id and msg_id:
                        new_text = (
                            f"📝 New Request\n"
                            f"• User: {req.get('user_name')}\n"
                            f"• Item: {req.get('description')}\n"
                            f"• Votes: 👍 {req.get('votes', 1)}"
                        )
        ...
        # Create a unique identifier for the request using current timestamp
        ...
        context.chat_data["requests_queue"] = queue
        # Inline keyboard for admin approval/rejection
        keyboard = InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton("✅ Approve", callback_data=f"request_approve_{req_id}"),
                    InlineKeyboardButton("❌ Reject", callback_data=f"request_reject_{req_id}"),
                ]
            ]
        )
        # Announce the new request in the chat with inline buttons; use HTML for mention
        text = (
            f"📝 New Request #{len(queue)}\n"
            f"• User: {requester.mention_html()}\n"
            f"• Item: {description}\n"
            f"• Votes: 👍 1"
        )
        try:
            ...
            # Store announcement message id for vote updates
            try:
                new_req["announce_chat_id"] = sent.chat.id
                new_req["announce_message_id"] = sent.message_id
                context.chat_data["requests_queue"] = queue
            except Exception:
                pass
        except Exception:
            # Fallback if mention_html fails
            await update.message.reply_text(
                f"📝 New Request #{len(queue)}\nUser: {requester.full_name}\nItem: {description}",
                reply_markup=keyboard,
            )
        # Log and inform the requester of their position
        add_log(context, f"REQUEST_ADD id={req_id} user={requester.id} item={description!r}")
        await reply_autodelete(
            update.message,
            context,
            f"Tumhara request queue me add ho gaya hai. Position: {len(queue)}.",
        )

    async def cmd_queue(update: Update, context: ContextTypes.DEFAULT_TYPE):
        """
        Display the current request queue. Only admins can view the full queue.
        Shows each request with its position, description, requester and status.
        """
        if not update.message:
            return
        # Restrict queue viewing to owner or group admins
        if update.effective_user.id != OWNER_ID and not await is_admin(update, context):
            return await reply_autodelete(update.message, context, "Only admins can view the request queue.")
        queue = context.chat_data.get("requests_queue", [])
        if not queue:
            return await reply_autodelete(update.message, context, "Request queue is empty.")
        lines = []
        for i, req in enumerate(queue, start=1):
            status = req.get("status", "pending")
            lines.append(f"{i}. {req['description']} (👍{req.get('votes', 1)}) — {req['user_name']} ({status})")
        await reply_autodelete(update.message, context, "Current Requests:\n" + "\n".join(lines))

    async def cmd_myrequests(update: Update, context: ContextTypes.DEFAULT_TYPE):
        """
        Allow a user to see their own pending/completed requests in the queue.
        """
        if not update.message:
            return
        ...
        for i, req in enumerate(queue, start=1):
            if req.get("user_id") == user_id:
                status = req.get("status", "pending")
                my_reqs.append(f"{i}. {req['description']} (👍{req.get('votes', 1)}) ({status})")
        if not my_reqs:
            return await reply_autodelete(update.message, context, "Aapki koi pending request nahi hai.")
        await reply_autodelete(update.message, context, "Aapki requests:\n" + "\n".join(my_reqs))

    async def cb_request(update: Update, context: ContextTypes.DEFAULT_TYPE):
        """
        Callback query handler for request approvals or rejections. Only admins can trigger these actions.
        On approval, the request status is updated, the requester is notified and the queue entry is removed.
        On rejection, the request is similarly updated and the requester is informed.
        """
        query = update.callback_query
        if not query:
            return
        data = query.data
        # Expected formats: request_approve_<id> or request_reject_<id>
        parts = data.split("_")
        if len(parts) < 3:
            return
        action = parts[1]
        ...
        if query.from_user.id != OWNER_ID and not await is_admin(update, context):
            return await query.answer("Only admins can process requests.", show_alert=True)
        queue = context.chat_data.get("requests_queue", [])
        for req in queue:
            if req.get("id") == req_id and req.get("status") == "pending":
                ...
                if action == "approve":
                    req["status"] = "approved"
                    add_log(context, f"REQUEST_APPROVED id={req_id} by={query.from_user.id}")
                    # Notify the requester
                    try:
                        await context.bot.send_message(
                            chat_id=req["user_id"],
                            text=f"✨ Tumhara request '{req['description']}' approve ho gaya!",
                        )
                    except Exception:
                        pass
                    # Update the inline message to reflect the decision
                    try:
                        await query.message.edit_text(
                            query.message.text + f"\n\n✔️ Approved by {query.from_user.full_name}",
                            parse_mode="HTML",
                        )
                    except Exception:
                        pass
                    ...
                    req["status"] = "rejected"
                    add_log(context, f"REQUEST_REJECTED id={req_id} by={query.from_user.id}")
                    try:
                        await context.bot.send_message(
                            chat_id=req["user_id"],
                            text=f"❌ Tumhara request '{req['description']}' reject kiya gaya.",
                        )
                    except Exception:
                        pass
                    try:
                        ...
                        pass
                # Remove the request from the queue to keep it tidy
                try:
                    queue.remove(req)
                except ValueError:
                    pass
...
def main():
    ...
    # Register request queue commands
    ...
    # Callback handler for request approvals/rejections
    application.add_handler(CallbackQueryHandler(cb_request, pattern=r"^request_(?:approve|reject)_"))
