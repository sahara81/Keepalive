...
def _set_onboarded(context: ContextTypes.DEFAULT_TYPE, user_id: int):
    _sql_set_onboarded(int(user_id))
...
# =========================
#  SQLITE PERSISTENCE (onboarded / blocked / cooldown / 48h limit)
# =========================
...
def _sql_init():
    ...
    try:
        cur = con.cursor()
        cur.execute("CREATE TABLE IF NOT EXISTS onboarded_users (user_id INTEGER PRIMARY KEY, ts INTEGER)")
        cur.execute("CREATE TABLE IF NOT EXISTS blocked_users (user_id INTEGER PRIMARY KEY, ts INTEGER)")
        cur.execute("CREATE TABLE IF NOT EXISTS cooldown (user_id INTEGER PRIMARY KEY, last_ts INTEGER)")
        cur.execute("CREATE TABLE IF NOT EXISTS req_limit (user_id INTEGER PRIMARY KEY, window_start INTEGER, count INTEGER)")
        con.commit()
    finally:
        con.close()

def _sql_onboarded(user_id: int) -> bool:
    _sql_init()
    con = _sql_conn()
    try:
        cur = con.cursor()
        cur.execute("SELECT 1 FROM onboarded_users WHERE user_id=?", (int(user_id),))
        return cur.fetchone() is not None
    finally:
        con.close()

def _sql_set_onboarded(user_id: int):
    _sql_init()
    con = _sql_conn()
    try:
        cur = con.cursor()
        cur.execute("INSERT OR REPLACE INTO onboarded_users(user_id, ts) VALUES(?, ?)", (int(user_id), int(time.time())))
        con.commit()
    finally:
        con.close()

def _sql_is_blocked(user_id: int) -> bool:
    _sql_init()
    con = _sql_conn()
    try:
        cur = con.cursor()
...
def _sql_block(user_id: int):
    _sql_init()
    con = _sql_conn()
    try:
        cur = con.cursor()
        cur.execute("INSERT OR REPLACE INTO blocked_users(user_id, ts) VALUES(?, ?)", (int(user_id), int(time.time())))
        con.commit()
    finally:
        con.close()

def _sql_cooldown_can_send(user_id: int) -> bool:
    ...
    try:
        cur = con.cursor()
        cur.execute("SELECT last_ts FROM cooldown WHERE user_id=?", (int(user_id),))
        row = cur.fetchone()
...
def _sql_cooldown_set(user_id: int):
    if COOLDOWN_SECONDS <= 0:
        return
    _sql_init()
    con = _sql_conn()
    try:
        cur = con.cursor()
        cur.execute("INSERT OR REPLACE INTO cooldown(user_id, last_ts) VALUES(?, ?)", (int(user_id), int(time.time())))
        con.commit()
    finally:
        con.close()

def _sql_limit_check_and_inc(user_id: int):
    _sql_init()
    now = int(time.time())
    con = _sql_conn()
    try:
        cur = con.cursor()
        cur.execute("SELECT window_start, count FROM req_limit WHERE user_id=?", (int(user_id),))
        row = cur.fetchone()
        if not row:
            ws, cnt = now, 0
        else:
            ws, cnt = int(row[0]), int(row[1])
        ...
        cnt2 = cnt + 1
        cur.execute("INSERT OR REPLACE INTO req_limit(user_id, window_start, count) VALUES(?, ?, ?)", (int(user_id), ws, cnt2))
        con.commit()
        return True, cnt2, ws
    finally:
        con.close()
...
def _t(lang: str, key: str, **kw) -> str:
    ...
    T = {
        ...
        "first_time_dm": {
            "hx": (
                "👋 First time setup\n\n"
                "Group clean rakhne ke liye messages DM me aate hain\n\n"
                "✅ Step 1: Isi PM me /start karo\n"
                "✅ Step 2: Phir group me jaakar /request dobara bhejo\n\n"
                "📌 Commands (Simple Guide)\n\n"
                "👥 Group:\n"
                "➡️ /request <item>\n"
                "   Example: /request Avengers\n"
                "   • Group message delete ho jayega\n"
                "   • Confirmation + status DM me milega\n\n"
                "📬 Private Chat:\n"
                "➡️ /myrequests\n"
                "   • Apni saari requests + status\n\n"
                "➡️ /help\n"
                "   • Ye guide dubara dekhne ke liye\n\n"
                "📊 Request status:\n"
                "✔️ Approved → fulfilled\n"
                "❌ Rejected → rejected\n"
                "• Har update tumhe DM me milega\n\n"
                "👉 Ab group me jaakar apna /request bhejo ✅"
            ),
            "hi": (
                "👋 First time setup\n\n"
                "Group clean rakhne ke liye messages DM me aate hain\n\n"
...
async def cmd_request(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # blocked user check
    if _sql_is_blocked(user.id):
        await _dm_safe(context, user.id, "Aap blocked ho. Aap request nahi kar sakte.")
        return

    # cooldown check (spam control)
    if not _sql_cooldown_can_send(user.id):
        await _dm_safe(context, user.id, "5 min baad request bhejo.")
        return

    # 48h max request limit (SQLite persistent)
    allowed, cnt_after, ws = _sql_limit_check_and_inc(user.id)
    if not allowed:
