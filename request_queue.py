...
async def cmd_request(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not (await _mongo_cooldown_can_send(user.id)) if _mongo_enabled() else _cooldown_ok(context, user.id):
        ok_dm = await _dm_safe(context, user.id, "5 min baad request bhejo.")
        if not ok_dm:
            await _send_group_hint(update, context, "5 min baad request bhejo.")
        return
    ...
    if _mongo_enabled():
        ...
        if not allowed:
            ok_dm = await _dm_safe(context, user.id, "Aapki 48 hours ki request limit full ho gayi hai. 48 hours baad try karo.")
            if not ok_dm:
                await _send_group_hint(update, context, "Aapki 48 hours ki request limit full ho gayi hai. 48 hours baad try karo.")
            return

    # short group reminder (auto-delete) to check DM
    mention = f"@{user.username}" if user.username else user.full_name
    await _send_group_hint(update, context, _t(lang, "check_dm_hint"))
    ...
    if not (await _mongo_onboarded(user.id) if _mongo_enabled() else _is_onboarded(context, user.id)):
        ...
        if ok:
            ...
            if _mongo_enabled():
                await _mongo_set_onboarded(user.id)
